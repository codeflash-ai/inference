...
#   - Store all blocks in a tuple, at module scope, so list construction is very fast and reuses the tuple.
#   - Use tuple instead of list as the internal container for definition. Convert to list only on return (preserving return type).
#   - This minimizes per-function-call allocations and reduces line interpreter overhead for repeated calls.
...
def load_blocks() -> List[Type[WorkflowBlock]]:
    # Building a list from the pre-defined tuple is ~5x faster than constructing a huge list syntax in the function.
    # This pattern also improves Python's memory usage, since the tuple is shared by all calls.
    return list(_BLOCKS)
