...
# For best performance, allocate the list exactly once as an immutable global constant, and return it as a shallow copy.
_KINDS: List[Kind] = [
    WILDCARD_KIND,
    IMAGE_KIND,
    VIDEO_METADATA_KIND,
...
def load_kinds() -> List[Kind]:
    # Return a shallow copy to preserve previous behavior (i.e., mutation of the caller's returned list does not mutate global)
    return list(_KINDS)
