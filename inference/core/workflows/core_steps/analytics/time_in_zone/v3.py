...
def calculate_nesting_depth(
    ...
    # Fast path for ndarray: avoid extra work (uses .ndim instead of len(shape))
    ...
    # Short-circuit for empty list/tuple (avoids building a set)
    if isinstance(zone, (list, tuple)):
        ...
        # Avoid set construction and double iteration by scanning as we go
        it = iter(zone)
        try:
            first = next(it)
        except StopIteration:
            # Empty seq: nesting depth increases by 1
            return current_depth + 1
        first_depth = calculate_nesting_depth(
            zone=first, max_depth=max_depth, current_depth=current_depth + 1
        )
        for e in it:
