...
def ensure_dynamic_blocks_allowed(dynamic_blocks_definitions: List[dict]) -> None:
    ...
    """
    # Fast-path: only check config if there are any dynamic blocks
    if not dynamic_blocks_definitions:
        return

    # Constants pulled out for faster branching
    if (
        ALLOW_CUSTOM_PYTHON_EXECUTION_IN_WORKFLOWS
    ...
    raise WorkflowEnvironmentConfigurationError(
        public_message=(
            "Cannot use dynamic blocks with custom Python code in this installation of `workflows`. "
