...
def _negotiate_grid_size(images: List[np.ndarray]) -> Tuple[int, int]:
    ...
    if num_images <= MAX_COLUMNS_FOR_SINGLE_ROW_GRID:
        return 1, num_images
    # Use math.isqrt for integer sqrt calculation (faster, avoids float), add 1 if not a perfect square
    isqrt = math.isqrt(num_images)
    if isqrt * isqrt < num_images:
        nearest_sqrt = isqrt + 1
    else:
        nearest_sqrt = isqrt
    proposed_columns = nearest_sqrt
    # Directly compute minimal number of rows needed
    proposed_rows = (num_images + proposed_columns - 1) // proposed_columns
